...
let default_error_handler com =
	(fun (err : Error.error) ->
		...
		(* While a speculative message capture is active the error has been diverted into the buffer;
		   defer setting has_error (and the fail-fast abort) until the buffer is committed, otherwise a
		   later-dropped attempt would still flag the whole compilation as failed. *)
