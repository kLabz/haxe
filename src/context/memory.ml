...
let display_memory com =
	...
	(* (1* print ("  haxelib " ^ size c.c_haxelib); *1) *)
	(* (1* print ("  parsed ast " ^ size c.c_files ^ " (" ^ string_of_int (Hashtbl.length c.c_files) ^ " files stored)"); *1) *)
	(* (1* print ("  typed modules " ^ size c.c_modules ^ " (" ^ string_of_int (Hashtbl.length c.c_modules) ^ " modules stored)"); *1) *)
