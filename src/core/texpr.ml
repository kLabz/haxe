...
(* [map_expr f e] applies [f] to the direct subexpressions of [e] and rebuilds [e].
   It preserves physical equality: when every mapped child is [==] to the original,
   the original node (and child lists) are returned unchanged instead of allocating a
   fresh copy. texpr is immutable, so this is semantically transparent, and it avoids
   copying the (usually large) unchanged majority of a body on every filter pass.
   Traversal order is unchanged (left-to-right, head-first). *)
let map_expr f e =
